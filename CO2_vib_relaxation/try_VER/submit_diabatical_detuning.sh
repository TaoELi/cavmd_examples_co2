...
#Global_path=/scratch/taoli/job/try_VER/N_10/
Global_path=./
RUN=vib_relax
...
for freq in 2100 2150 2450 2500 2000 2200 2250 2300 2350 2400 2600
do
    cd $PBS_O_WORKDIR
    
    DIR=$Global_path/Freq_$freq
    mkdir -p $DIR
    cp data.lmp photon_params.json modified_configs_exc_10/init_* input_traj.xml.bak  in.lmp $DIR
    cd $DIR
    echo "move in $DIR"
    
    sed -i "s/0e-4/$E0/" photon_params.json
  ...
  (  	echo "Dealing with $traj slice"
        ...
        sed -i "s/mesitylene-pimd.1/$RUN.Freq_$freq.traj_$traj/" input_traj_$traj.xml
        sed -i "s/mesitylene-pimd.1/$RUN.Freq_$freq.traj_$traj/" in_$traj.lmp
