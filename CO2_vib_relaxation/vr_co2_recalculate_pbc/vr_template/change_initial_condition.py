# usage: python change_initial_condition.py Nhot
# Nhot = 10, 20 represents the number of hot molecules in the simulation cell
# Output: create a dir=modified_configs_exc_Nhot and store the modified chk files
...
XML_DIR = sys.argv[-2]
print("We are dealing with folder", XML_DIR)
N_MODIFIED = int(sys.argv[-1])
...
def operate_all_files(xml_dir):
    xml_files = glob.glob(xml_dir + "/init*checkpoint")
    store_folder="modified_configs_exc_%d/" %N_MODIFIED
    # Try to create this store folder if there is no this folder
    from pathlib import Path
    Path(store_folder).mkdir(parents=True, exist_ok=True)
    # loop over all files
    for xml_file in xml_files:
        #print("dealing with", xml_file)
        operate_one_file(xml_path=xml_file, store_folder=store_folder)


if __name__ == "__main__":
    operate_all_files(XML_DIR)
