...
def extract_ph_decay_rates(paths, omega_lst, PlotIt=False, nstart=300):
    ks = []
    for omega, path in zip(omega_lst, paths):
        ...
        try:
            ...
            yfit_ph, tau = fit_exponential(t[nstart:], ph[nstart:])
...
if __name__ == '__main__':
    paths = sys.argv[1:]
    extract_ph_decay_rates(paths=paths, omega_lst=[2320]*len(paths), PlotIt=True)
