...
def plot_ph_dynamics():
    ...
    cav_lifetimes = [300, 600, 1000, 5000]
    paths_LP_cavloss = ["pumping_cavloss/E0_2e-4_Exc_LP_CavLoss_%d" %t for t in cav_lifetimes] + ["pumping_Rabi/E0_2e-4_Exc_LP_Amp_6e-4/"]
    paths_UP_cavloss = ["pumping_cavloss/E0_2e-4_Exc_UP_CavLoss_%d" %t for t in cav_lifetimes] + ["pumping_Rabi/E0_2e-4_Exc_UP_Amp_6e-4/"]
    cav_loss_array = 1.0 / np.array(cav_lifetimes + [1e50]) * 1e3
    ...
    colors0 = ['k']*n_data
    labels = ["$\\tau_{cav} = %.1f $ ps" %(t/1e3) for t in cav_lifetimes] + ["$\\tau_{cav} = \infty$"]

    clp.plotone(xs_UP, ys_UP, axes[0], colors=colors0, labels=labels,  lw=0.9, xlabel="time [ps]", ylabel="photonic energy [$\hbar\omega_c$]", xlim=[0, 5], ylim=[0, 0.013/au2omega0], showlegend=False)
    ...
    clp.plotone([freq2, freq], [sp2, sp+1.5], axins, colors=['k', 'r'], lw=1., showlegend=False, xlim=[2200, 2510], ylim=[-0.03, 4])
    axins.axvline(x=2320, color='b', linestyle="--", lw=0.4)
    ...
    clp.plotone(xs_LP, ys_LP, axes[1], colors=colors0, labels=labels,  lw=0.9, xlabel="time [ps]", ylabel="photonic energy [$\hbar\omega_c$]", xlim=[0, 5], ylim=[0, 0.0078/au2omega0], showlegend=False)
    ax = axes[1]
    colors = [colormap(i) for i in np.linspace(0, 0.6,len(ax.lines))]
    for i,j in enumerate(ax.lines):
        j.set_color(colors[i])
    ...
    clp.adjust(tight_layout=False, savefile="decay_dynamics_cavloss.pdf", includelegend=lgd)
