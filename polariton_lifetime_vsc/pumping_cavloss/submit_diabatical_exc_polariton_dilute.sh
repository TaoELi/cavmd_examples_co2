...
Global_path=/scratch/taoli/job/polariton_lifetime_vsc/pumping_cavloss/
RUN=cavloss
...
do
for CavLoss in 300 600 1000 3000 5000 10000 1000000
do
    for idx in ${!E0_lst[@]}
    do
	...
	DIR=$Global_path/E0_$E0\_Exc_$Polariton\_CavLoss_$CavLoss\_dilute
	...
	echo "E0 = $E0, PolaritonFreq = $PolaritonFreq"
        ...
        cd $PBS_O_WORKDIR
        mkdir -p $DIR
        cp data.lmp photon_params.json ../eq_density/box_60/E0_$E0/init_* input_traj.xml.bak  in.lmp $DIR
        cd $DIR
        echo "move in $DIR"
	...
	# modify cavity loss
	sed -i "s/1000000/$CavLoss/" input_traj.xml.bak
