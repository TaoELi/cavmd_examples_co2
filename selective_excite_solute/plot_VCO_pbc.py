...
def obtain_VCO_trajectories_usr_function(path, pattern="simu_*.out"):
    data = obtain_avg_data(path=path, pattern=pattern)
    t = data[:,1]
    e1 = data[:, 3] - 0.000950037256
    # last molecule is 13CO2
    e2 = data[:, 4] - 0.000950037256
    au2cminv = 219474.63
    return t, e1 * au2cminv, e2 * au2cminv
...
def obtain_pbc_data():
    ...
    for N in Ns:
        t, e1, e2 = obtain_VCO_trajectories_usr_function(path="pumping_LP_simulation_pbc/pump_N_%d_many13CO2/" %N)
        ts.append(t)
        e1s.append(e1)
        e2s.append(e2)
    return ts, e1s, e2s
...
def plot_total():
    ...
    ax.annotate(s='', xy=(arrowx, e1s[0][nsize]), xytext=(arrowx,e2s[0][nsize]), arrowprops=dict(arrowstyle='<->'))
    #ax.text(arrowx+0.5, ymax*0.25, '%dX' %(e2s[-1][nsize]/e1s[-1][nsize]))
    ...
    ax.text(0.4, 0.7, "solute (4.63\% $^{13}$CO$_2$)", fontsize=10, transform=ax.transAxes, color='blue')

    clp.adjust(savefile="VCO_pbc.pdf", tight_layout=True, includelegend=lgd)
