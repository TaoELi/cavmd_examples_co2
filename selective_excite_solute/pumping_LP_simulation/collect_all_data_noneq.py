...
class MD_Analysis:
    ...
    def cacl_CO_energy_traj_timespan(self, ntstart=0, ntend=10001, nskipping=1, nmol=216, summed=False, sublist=None):
        ...
        # we have deleted the equilibrium length
        CO_1 = np.sqrt(np.sum(np.abs(C_traj - O1_traj)**2, axis=1)) * 1.8897259885789 - 2.196 # to a.u.
        CO_2 = np.sqrt(np.sum(np.abs(C_traj - O2_traj)**2, axis=1)) * 1.8897259885789 - 2.196 # to a.u.
        def VCO(x):
    ...
    def output_VCO_dynamics(self, ntstart=0, ntend=10001, nskipping=1, summed=False, nmol=216, sublist=None):
        ...
        if os.path.isfile(local_filename + "dd"):
            ...
            time_fs, VCO_traj = self.cacl_CO_energy_traj_timespan(ntstart=ntstart,
                ntend=ntend, nskipping=nskipping, summed=summed, nmol=216, sublist=sublist)
    ...
    for i, path in enumerate(paths):
        ...
        for j, filename in enumerate(filenames):
            if os.path.getsize(filename) >= default_file_size:
                ...
                a.output_VCO_dynamics(ntstart=0, ntend=10001, nskipping=10)
                a.output_ph_energy_dynamics(ntstart=0, ntend=10001)
                if filename.split("/")[-1] == "simu_1.xc.xyz":
                    n = 500
